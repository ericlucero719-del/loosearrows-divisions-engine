...
function calculateProfit(items, skuMatches) {
    let profit = 0;
    for (const item of items) {
...
async function createOrderRecord(order, skuMatch, profitUsd) {
    return prisma.tikTokOrder.create({
        data: {
...
async function selectVendor(items) {
    ...
    const preferred = vendors.find(v => {
        const cats = JSON.parse(v.categoriesJson || "[]");
...
async function runComplianceCheck(vendor) {
...
async function sendPO(tikTokOrderId, order, vendor, compliance) {
...
async function generateLabel(order_id) {
...
async function pushTracking(order_id, carrier = "UPS", trackingRef) {
    const ref = trackingRef ?? `1Z-TT-${Date.now().toString(36).toUpperCase()}`;
    const tikOrder = await prisma.tikTokOrder.findUnique({ where: { externalId: order_id } });
...
async function supplierFulfill(order_id) {
    const tikOrder = await prisma.tikTokOrder.findUnique({ where: { externalId: order_id } });
    if (tikOrder?.poId) {
        const count = await prisma.govShipment.count();
        const shipRef = `TT-SHIP-${String(count + 1).padStart(3, "0")}`;
...
async function syncPayment(invoice) {
    ...
    await prisma.tikTokOrder.updateMany({
        where: { invoiceId: invoice.invoiceId },
        data: { status: "PAID" },
    });
}
// ─── 10. Notification Log — appended to order notes ───────────────────────────
async function sendNotification(order_id, event) {
    const note = `[${new Date().toISOString()}] EVENT: ${event}`;
    const existing = await prisma.tikTokOrder.findUnique({ where: { externalId: order_id } });
    const notes = existing?.notes ? `${existing.notes}\n${note}` : note;
    await prisma.tikTokOrder.update({ where: { externalId: order_id }, data: { notes } });
}
// ─── 11. Inventory Update — adjusts Division 1 stock notes ───────────────────
async function updateInventory(order_id) {
    const tikOrder = await prisma.tikTokOrder.findUnique({ where: { externalId: order_id } });
    if (!tikOrder)
        return;
    const items = JSON.parse(tikOrder.itemsJson || "[]");
    for (const item of items) {
        ...
        if (product) {
            ...
            await prisma.govProduct.update({ where: { sku: item.sku }, data: { notes: note } });
        }
    }
}
...
class TikTokService {
    ...
    async fulfill(order_id, method, carrier, trackingRef) {
        if (method === "home") {
    ...
    async recordPayment(order_id) {
        const tikOrder = await prisma.tikTokOrder.findUnique({ where: { externalId: order_id } });
        if (!tikOrder?.invoiceId)
            throw new Error("No invoice found for this order. Run /tiktok/invoice first.");
        const inv = await prisma.govInvoice.findUnique({ where: { invoiceId: tikOrder.invoiceId } });
    ...
    async listOrders(status) {
        ...
        const rows = await prisma.tikTokOrder.findMany({ where, orderBy: { createdAt: "desc" } });
        return rows.map((r) => ({
            orderId: r.orderId,
    ...
    async getOrder(order_id) {
        const row = await prisma.tikTokOrder.findUnique({ where: { externalId: order_id } });
        if (!row)
