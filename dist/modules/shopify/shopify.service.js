...
const svc = commerce_service_1.CommerceService.forPlatform({
    platform: "SHOPIFY",
    prefix: "SHO",
...
async function shopifyGet(path) {
    ...
    if (!res.ok || data.errors) {
        throw new Error(`Shopify API error ${res.status} for store "${STORE}". ` +
...
exports.shopifyService = {
    ...
    async syncOrders(limit = 50) {
        if (!TOKEN || !STORE)
            throw new Error("SHOPIFY_ACCESS_TOKEN and SHOPIFY_STORE_DOMAIN must be set");
    ...
    async processWebhook(topic, body) {
