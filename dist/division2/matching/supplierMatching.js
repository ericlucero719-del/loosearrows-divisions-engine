...
exports.matchSupplier = matchSupplier;
const WEIGHTS = {
    cost: 0.4,
    speed: 0.25,
    stock: 0.15,
...
function getPriorityBoost(supplierId, priorityRules) {
...
async function matchSupplier(input, attempt, options) {
    ...
    const scored = suppliers.map((supplier, index) => {
        ...
        const baseScore = normCost[index] * WEIGHTS.cost +
    ...
    const triedSupplierIds = [];
    let lastError = null;
    for (const entry of ranked) {
        const supplier = entry.supplier;
        if (entry.rationale.stockScore === 0)
        ...
        if (attemptSuccess && attemptResult !== undefined) {
            const preferredContact = supplier.apiEndpoint ? "api" : supplier.portalUrl ? "portal" : "email";
            const result = {
    ...
    return {
        failure: {
            reason: "api-failure",
            message: "All selected suppliers failed after retrying. Manual review required.",
            attemptedSupplierIds: triedSupplierIds,
        },
    };
}
//# sourceMappingURL=supplierMatching.js.map