...
exports.createScoringEngine = createScoringEngine;
function createScoringEngine({ logger, db, contractData, }) {
    function score(event) {
        ...
        const contractAligned = !!(event.relatedContractId &&
            contractData.contracts[event.relatedContractId]);
        if (contractAligned) {
            delta += 3;
            flags.push("CONTRACT_ALIGNED");
        }
