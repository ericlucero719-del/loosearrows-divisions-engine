...
async function requireApiKey(req, res, next) {
    ...
    if (PUBLIC_EXACT.has(p))
        return next();
    if (PUBLIC_PREFIX.some(prefix => p.startsWith(prefix)))
        return next();
    ...
    req.apiTier = tier;
    req.apiKeyTier = tier; // alias used by rate limiter
    req.apiKeyId = record.id; // used as rate-limit bucket key
    req.apiKeyOwner = record.ownerName;
    next();
}
