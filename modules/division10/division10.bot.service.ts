...
import { division2Service } from "../division2/division2.service";
import { division3Service } from "../division3/division3.service";
import {
...
const ALERT_LOGIC: Record<AlertType, AlertSeverity> = {
...
function isInScope(naics?: string, psc?: string): boolean {
  if (!naics && !psc) return true;
  if (naics && DIV10_NAICS.includes(naics)) return true;
  if (psc   && DIV10_PSC.includes(psc))     return true;
  return false;
}

function fitBandFor(score: number): FitBand {
  if (score >= 0.80) return "strong";
  if (score >= 0.50) return "moderate";
  return "weak";
}

function marginBandFor(m: number): BotDraftQuote["marginBand"] {
...
function stepDiscover(): { discovered: number; alreadyKnown: number } {
  ...
  contracts.forEach(c => {
    ...
    if (known) { alreadyKnown++; return; }
    if (!isInScope(c.naics, c.psc)) return;
    ...
    emitRelic("discovery", opp.agency,
      `New opportunity discovered: ${opp.solicitationNumber} — ${opp.title}`,
      opp.oppId,
      { solNum: opp.solicitationNumber, naics: opp.naics, psc: opp.psc, clinCount: clins.length });

    raiseAlert("new-opportunity", opp.agency,
      `New contract discovered — ${opp.solicitationNumber}: ${opp.title}`,
      "Review opportunity and authorize analysis.",
      opp.oppId);
  });
  ...
  bids.forEach((b: any) => {
    ...
    emitRelic("discovery", opp.agency,
      `Pipeline bid ingested: ${opp.solicitationNumber} — ${b.status}`,
      opp.oppId,
...
function stepClassify(opp: BotOpportunity): void {
  opp.status    = "CLASSIFIED";
  opp.updatedAt = new Date().toISOString();
  emitRelic("analysis", opp.agency,
    `Classified: ${opp.solicitationNumber} — NAICS ${opp.naics ?? "?"} PSC ${opp.psc ?? "?"}`,
    opp.oppId,
    { naics: opp.naics, psc: opp.psc, source: opp.source });
}

// ── STEP 3: EXTRACT ───────────────────────────────────────────────────────────

function stepExtract(opp: BotOpportunity): void {
  ...
  if (missingCLINs.length) {
    raiseAlert("missing-info", opp.agency,
      `${missingCLINs.length} CLIN(s) missing description in ${opp.solicitationNumber}: ${missingCLINs.map(c => c.clin).join(", ")}`,
      "Retrieve full solicitation document and populate CLIN details before analysis.",
      opp.oppId);
  }

  // Check missing set-aside
  if (!opp.setAside) {
    raiseAlert("compliance-risk", opp.agency,
      `Set-aside type not specified for ${opp.solicitationNumber}. Cannot verify eligibility.`,
      "Confirm set-aside status from solicitation. Required before any submission.",
      opp.oppId);
  ...
  opp.updatedAt = new Date().toISOString();
  emitRelic("analysis", opp.agency,
    `Extracted ${opp.clins.length} CLINs from ${opp.solicitationNumber}. ${missingCLINs.length} gap(s) flagged.`,
    opp.oppId,
    { clinCount: opp.clins.length, missingCLINs: missingCLINs.length });
}

// ── STEP 4: ANALYZE ───────────────────────────────────────────────────────────

function stepAnalyze(opp: BotOpportunity): void {
  ...
  if (prices.length >= 2) {
    ...
    if (maxDev > 0.30) {
      ...
      raiseAlert("high-variance-price", opp.agency,
        `Price variance of ${(maxDev * 100).toFixed(0)}% detected in ${opp.solicitationNumber}. Review CLIN pricing before draft.`,
        "Architect to review pricing spread before authorizing submission.",
        opp.oppId);
      raw -= 0.05;
    }
  }
  ...
  if (fitBand === "weak") {
    raiseAlert("fit-warning", opp.agency,
      `Fit score ${fitScore.toFixed(2)} (WEAK) for ${opp.solicitationNumber}. Low alignment with Division 10 scope.`,
      "Architect to decide whether to continue or close opportunity.",
      opp.oppId);
  }
  ...
  emitRelic("analysis", opp.agency,
    `Analysis complete — ${opp.solicitationNumber} | Fit: ${fitScore.toFixed(2)} (${fitBand.toUpperCase()})`,
    opp.oppId,
    { fitScore, fitBand, riskFlags: riskFlags.length, complianceRisks: opp.complianceRisks?.length });
}

// ── STEP 5: MATCH ─────────────────────────────────────────────────────────────

function stepMatch(opp: BotOpportunity): void {
  ...
  const recs: SupplierRec[] = vendors
    ...
    .map(v => {
      ...
      const tier: SupplierTier = composite >= 80 ? "primary" : composite >= 60 ? "backup" : "not-recommended";

      // Flag unreliable vendors
      const relNorm = reliability / 100;
      if (relNorm < 0.75) {
        raiseAlert("supplier-issue", v.name ?? v.vendorName ?? "Unknown Vendor",
          `Vendor reliability ${(relNorm * 100).toFixed(0)}% is below 75% threshold for ${opp.solicitationNumber}.`,
          "Consider backup supplier. Notify Architect before allocating to primary.",
          opp.oppId);
      }
  ...
  if (!recs.length) {
    raiseAlert("supplier-issue", opp.agency,
      `No vendors registered for opportunity ${opp.solicitationNumber}. Supplier match incomplete.`,
      "Register capable vendors in Division 7 before draft preparation.",
      opp.oppId);
  }
  ...
  emitRelic("match", opp.agency,
    `Supplier match complete — ${opp.solicitationNumber} | ${recs.length} vendor(s) | Top: ${recs[0]?.vendorName ?? "none"} (${recs[0]?.score ?? 0}/100, ${recs[0]?.tier ?? "-"})`,
    opp.oppId,
    { vendorCount: recs.length, topScore: recs[0]?.score, topVendor: recs[0]?.vendorName, topTier: recs[0]?.tier });
}

// ── STEP 6: DRAFT ─────────────────────────────────────────────────────────────

function stepDraft(opp: BotOpportunity): BotDraftQuote {
  const products = Object.values(registry.products ?? {}) as any[];
  const riskFlags: string[] = [
  ...
  const lineItems: BotQuoteLineItem[] = opp.clins.map(clin => {
    ...
    const source: BotQuoteLineItem["priceSource"] = clin.unitPrice && clin.unitPrice > 0
  ...
  emitRelic("prep", opp.agency,
    `Draft prepared — ${opp.solicitationNumber} | $${totalEstimate.toFixed(2)} | ${draft.marginBand} margin | READY_FOR_ARCHITECT_REVIEW`,
    opp.oppId,
    { totalEstimate, marginBand: draft.marginBand, riskFlagCount: riskFlags.length, clinCount: lineItems.length });
...
function stepAlert(): number {
  ...
  Object.values(botState.opportunities).forEach(opp => {
    ...
    const exists = (t: string) =>
      botState.alerts.some(a => a.oppId === opp.oppId && a.alertType === t && !a.acknowledged);

    if (hrs > 0 && hrs <= DL.h24 && !exists("deadline-24h")) {
      raiseAlert("deadline-24h", entity,
        `DEADLINE IN ${hrs.toFixed(1)}h — ${opp.solicitationNumber}: ${opp.title}`,
        "IMMEDIATE Architect action required. Bot cannot submit.",
        opp.oppId);
      raised++;
    } else if (hrs > DL.h24 && hrs <= DL.h48 && !exists("deadline-48h")) {
      raiseAlert("deadline-48h", entity,
        `Deadline in ${hrs.toFixed(0)}h — ${opp.solicitationNumber}: ${opp.title}`,
        "Architect review required. Ensure draft is prepared and approved.",
        opp.oppId);
      raised++;
    } else if (hrs > DL.h48 && hrs <= DL.h72 && !exists("deadline-72h")) {
      raiseAlert("deadline-72h", entity,
        `Deadline in ${hrs.toFixed(0)}h — ${opp.solicitationNumber}: ${opp.title}`,
...
function stepRecommend(opp: BotOpportunity): BotRecommendation {
  ...
  let decision: BotRecommendation["decision"];
  ...
  emitRelic("recommendation", opp.agency,
    `Recommendation: ${decision} — ${opp.solicitationNumber} | Fit ${fitScore.toFixed(2)} (${fitBand}) | Supplier ${suppScore}/100`,
    opp.oppId,
    { decision, fitScore, fitBand, suppScore, marginBand: rec.marginBand });
...
function stepEscalate(opp: BotOpportunity): void {
  const rec = opp.recommendation;
  if (!rec || rec.decision === "DO_NOT_PURSUE") return;

  opp.status    = "ESCALATED";
  opp.updatedAt = new Date().toISOString();

  raiseAlert("escalation", opp.agency,
    `${rec.decision} — ${opp.solicitationNumber} is READY FOR ARCHITECT REVIEW. Fit ${(opp.fitScore ?? 0).toFixed(2)} (${opp.fitBand}). Estimated value: $${opp.draftQuote?.totalEstimate?.toFixed(2) ?? "TBD"}. Deadline: ${rec.deadlineReminder}.`,
    `Architect must review analysis, supplier match, and draft quote. Bot awaits explicit authorization. NO SUBMISSION HAS BEEN MADE.`,
    opp.oppId);

  emitRelic("alert", opp.agency,
    `Step 10 — AWAITING ARCHITECT APPROVAL: ${opp.solicitationNumber} | ${rec.decision} | $${opp.draftQuote?.totalEstimate?.toFixed(2) ?? "TBD"}`,
    opp.oppId,
    { step: 10, decision: rec.decision, awaitingApproval: true });
}

// ── FULL 10-STEP CYCLE ────────────────────────────────────────────────────────

function runCycle(cycleType: BotCycleSummary["cycleType"] = "manual"): BotCycleSummary {
  ...
  toProcess.forEach(opp => {
    botState.loopStep = "2_CLASSIFY";   stepClassify(opp);
    botState.loopStep = "3_EXTRACT";    stepExtract(opp);
    botState.loopStep = "4_ANALYZE";    stepAnalyze(opp);
    botState.loopStep = "5_MATCH";      stepMatch(opp);
    if ((opp.fitScore ?? 0) >= 0.50) {
      botState.loopStep = "6_DRAFT";    stepDraft(opp);
    }
    botState.loopStep = "7_ALERT";
    // deadline check happens globally below
    botState.loopStep = "8_RECOMMEND";  stepRecommend(opp);
  ...
  emitRelic("alert", "SYSTEM",
    `Cycle ${cycleId.slice(0, 8)} [${cycleType}] — ${relicsInCycle} relics | ${alertsInCycle} alerts | ${escalations.length} escalations`,
    undefined,
    { cycleId, cycleType, relicsCreated: relicsInCycle, alertsRaised: alertsInCycle, escalations });
...
function buildDailySummary(): DailyIntelligenceSummary {
  ...
  const recs: string[] = [];
  if (ready > 0)   recs.push(`${ready} opportunity(ies) awaiting Architect review.`);
  if (critical > 0) recs.push(`${critical} unacknowledged high-severity alert(s) require attention.`);
  if (soon.length)  recs.push(`${soon.length} deadline(s) within 7 days — prioritize review.`);

  emitRelic("recommendation", "SYSTEM",
    `Daily intelligence summary — ${open} open | ${ready} ready | ${critical} critical alerts`,
    undefined,
    { open, ready, critical, upcomingDeadlines: soon.length });
...
function buildWeeklyReport(): WeeklyDivision10Report {
  const opps   = Object.values(botState.opportunities);
  ...
  const relicTypeCounts = new Map<BotRelicType, number>();
  ...
  const alertTypeCounts = new Map<AlertType, number>();
  ...
  opps.forEach(o => {
    ...
    cats.forEach(cat => {
      if (!catMap.has(cat)) catMap.set(cat, { count: 0, scores: [] });
      catMap.get(cat)!.count++;
      if (o.fitScore) catMap.get(cat)!.scores.push(o.fitScore);
    });
  });
  ...
  const report: WeeklyDivision10Report = {
  ...
  emitRelic("recommendation", "SYSTEM",
    `Weekly Division 10 report — ${opps.length} opps | ${relics.length} relics | ${pursued} pursued`,
    undefined,
    { totalOpportunities: opps.length, pursuedCount: pursued, totalRelics: relics.length });
...
async function buildPipeline(opp: BotOpportunity): Promise<{ contractId: string; bidId: string; bidRef: string } | { error: string }> {
  try {
    ...
    for (const clin of opp.clins) {
      ...
      await division2Service.addProductToContract(contract.contractId, {
        sku:           clin.clin,
        clin:          clin.clin,
    ...
    const bid = await division3Service.createBid({
    ...
    if ("error" in bid) return { error: bid.error };
...
async function issueArchitectCommand(
  oppId:   string,
  command: ArchitectCommand,
  notes?:  string,
): Promise<ArchitectCommandLog | { error: string }> {
  const opp = botState.opportunities[oppId];
  if (!opp) return { error: "Opportunity not found" };
  ...
  switch (command) {
    case "Proceed": {
      ...
      raiseAlert("escalation", opp.agency,
        `PROCEED — ${opp.solicitationNumber} pipeline ready. Bid ${opp.pipelineRef?.bidRef ?? "pending"} awaiting Architect review and submission authorization.`,
        `Review bid ${opp.pipelineRef?.bidId ?? "N/A"} at GET /division/3/bids/{id} then POST /division/3/bids/{id}/submit when ready.`,
        opp.oppId);
      break;
    }
...
export const botService = {
  ...
  ingestOpportunity(data: {
    ...
    botState.opportunities[opp.oppId] = opp;
    emitRelic("discovery", opp.agency,
      `Manual ingest: ${opp.solicitationNumber} — ${opp.title}`,
      opp.oppId,
      { solNum: opp.solicitationNumber, source: opp.source });
    raiseAlert("new-opportunity", opp.agency,
      `Opportunity ingested: ${opp.solicitationNumber} — ${opp.title}`,
      "Authorize analysis cycle. Escalate to Architect when ready.",
      opp.oppId);
    return opp;
  },
  ...
  getRelics(type?: BotRelicType, oppId?: string, limit?: number): BotRelic[] {
    ...
    if (oppId) r = r.filter(x => x.oppId === oppId);
    return limit ? r.slice(0, limit) : r;
  },

  getAlerts(level?: AlertSeverity, acknowledged?: boolean): BotAlert[] {
    let a = [...botState.alerts].reverse();
    if (level !== undefined)        a = a.filter(x => x.severity    === level);
    if (acknowledged !== undefined) a = a.filter(x => x.acknowledged === acknowledged);
    return a;
  },
  ...
  acknowledgeAlert(alertId: string): BotAlert | { error: string } {
    ...
    emitRelic("update", "SYSTEM",
      `Alert acknowledged by Architect: [${alert.alertType}] ${alert.message.slice(0, 80)}`,
      alert.oppId,
      { alertId, alertType: alert.alertType });
    return alert;
  },
