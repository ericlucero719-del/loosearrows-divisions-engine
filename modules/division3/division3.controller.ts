...
export const division3Controller = {
  ...
  updatePricing(req: Request, res: Response) {
    const { prices } = req.body;
    if (!Array.isArray(prices) || prices.length === 0) {
      return res.status(400).json({ error: "prices array is required" });
    }
    const result = division3Service.updatePricing(req.params.id, prices);
    if ("error" in result) return res.status(400).json(result);
    return res.json(result);
  },

  // GET /division/3/bids/:id/submission  — HTML capability statement
  getSubmissionDoc(req: Request, res: Response) {
    const bid = division3Service.getBid(req.params.id) as any;
