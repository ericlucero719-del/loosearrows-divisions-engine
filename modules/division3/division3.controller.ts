...
export const division3Controller = {
  ...
  async createRequest(req: Request, res: Response) {
    ...
    return res.status(201).json(await division3Service.createRequest({ type, requestorId, notes }));
  },

  async attachProducts(req: Request, res: Response) {
    const { productIds } = req.body;
    if (!Array.isArray(productIds)) {
      return res.status(400).json({ error: "productIds must be an array" });
    }
    const result = await division3Service.attachProducts(req.params.id, productIds);
    if (!result) return res.status(404).json({ error: "Request not found" });
    return res.json(result);
  },

  async linkContract(req: Request, res: Response) {
    const { contractId } = req.body;
    if (!contractId) return res.status(400).json({ error: "contractId is required" });
    const result = await division3Service.linkContract(req.params.id, contractId);
    if (!result) return res.status(404).json({ error: "Request not found" });
    return res.json(result);
  },
  ...
  async updateStatus(req: Request, res: Response) {
    ...
    const result = await division3Service.updateStatus(req.params.id, status);
    if (!result) return res.status(404).json({ error: "Request not found" });
    return res.json(result);
  },

  async listRequests(_req: Request, res: Response) {
    return res.json(await division3Service.listRequests());
  },

  async getRequest(req: Request, res: Response) {
    const result = await division3Service.getRequest(req.params.id);
    if (!result) return res.status(404).json({ error: "Request not found" });
    return res.json(result);
  },
  ...
  async getBidPipeline(_req: Request, res: Response) {
    return res.json(await division3Service.getBidPipeline());
  },
  ...
  async createBid(req: Request, res: Response) {
    ...
    const result = await division3Service.createBid({ contractId, vendorId, vendorName, requestId, lineItems, notes, bidRef });
  ...
  async listBids(req: Request, res: Response) {
    ...
    return res.json(await division3Service.listBids(status as any));
  },

  // GET /division/3/bids/:id
  async getBid(req: Request, res: Response) {
    const result = await division3Service.getBid(req.params.id);
  ...
  async setLineItems(req: Request, res: Response) {
    const { lineItems } = req.body;
    if (!Array.isArray(lineItems)) {
      return res.status(400).json({ error: "lineItems must be an array of { sku, quantity, unitPrice, clin?, description? }" });
    }
    const result = await division3Service.setLineItems(req.params.id, lineItems);
    if ("error" in result) return res.status(400).json(result);
    return res.json(result);
  },
  ...
  async generateQuote(req: Request, res: Response) {
    const result = await division3Service.generateQuote(req.params.id);
    if ("error" in result) return res.status(400).json(result);
    return res.json(result);
  },
  ...
  async submitBid(req: Request, res: Response) {
    const result = await division3Service.submitBid(req.params.id);
    if ("error" in result) return res.status(400).json(result);
    return res.json(result);
  },
  ...
  async updateBidStatus(req: Request, res: Response) {
    const { status } = req.body;
    if (!status) return res.status(400).json({ error: "status is required" });
    const result = await division3Service.updateBidStatus(req.params.id, status);
    if ("error" in result) return res.status(400).json(result);
    return res.json(result);
  },
  ...
  async updatePricing(req: Request, res: Response) {
    ...
    const result = await division3Service.updatePricing(req.params.id, prices);
    if ("error" in result) return res.status(400).json(result);
    return res.json(result);
  },

  // GET /division/3/bids/:id/submission  — HTML capability statement
  async getSubmissionDoc(req: Request, res: Response) {
    const bid = await division3Service.getBid(req.params.id) as any;
    if (!bid || "error" in bid) return res.status(404).json({ error: "Bid not found" });

    const contract = bid._contract as any;
    ...
    const lineRows = bid.lineItems.map((li: any, i: number) => {
