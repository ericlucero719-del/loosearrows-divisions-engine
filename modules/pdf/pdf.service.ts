...
function header(doc: InstanceType<typeof PDFDocument>, title: string, refNo: string) {
...
function sectionLabel(doc: InstanceType<typeof PDFDocument>, label: string) {
  doc.moveDown(0.6);
...
function kv(doc: InstanceType<typeof PDFDocument>, key: string, value: string, col = MARGIN, colW = 220) {
  const y = doc.y;
...
function lineItemsTable(
  ...
  items: Array<{ clin?: string | null; sku?: string | null; description?: string | null; quantity: number; unitPrice: number; extended: number }>,
) {
...
function footer(doc: InstanceType<typeof PDFDocument>) {
  ...
  doc.font(FONT_REG).fontSize(7).fillColor(C.midGray)
     .text(
       "Loose Arrows Supply & Logistics™  |  CAGE: TBD  |  UEI: TBD  |  This document is confidential.",
       MARGIN, y + 6, { align: "center", width: w - MARGIN * 2 },
     );
}
...
export async function generateInvoicePdf(invoiceId: string, res: Response) {
  const inv = await (prisma as any).govInvoice.findUnique({
    where:   { invoiceId },
  ...
  if (inv.notes) { sectionLabel(doc, "Notes"); doc.font(FONT_REG).fontSize(9).fillColor(C.darkGray).text(inv.notes, MARGIN); }

  footer(doc);
  doc.end();
}

export async function generatePoPdf(poId: string, res: Response) {
  const po = await (prisma as any).govPO.findUnique({
    where:   { poId },
    include: { lineItems: true },
  });
  ...
  if (po.notes) { sectionLabel(doc, "Notes"); doc.font(FONT_REG).fontSize(9).fillColor(C.darkGray).text(po.notes, MARGIN); }

  footer(doc);
  doc.end();
}

export async function generateCapabilityStatementPdf(bidId: string, res: Response) {
  const bid = await (prisma as any).govBid.findUnique({
    where:   { bidId },
